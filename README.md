...
- **底池系统**: 自动计算底池和当前下注额
- **计时器**: 可配置操作超时（默认30秒），自动弃牌
- **断线重连**: 支持玩家断线后重新连接，当前行动玩家断线会自动按弃牌推进
- **游戏配置**: 房主可设置最小下注、操作时间、初始筹码
- **美观UI**: 专业扑克桌设计，流畅动画
...
> 注意：当前版本暂不支持 All-in / Side Pot。若筹码不足以完成跟注，系统会拒绝该操作。
...
| updateSettings | { settings } | 更新游戏配置（仅房主） |
...
| settingsUpdated | { settings } | 游戏配置已更新 |
